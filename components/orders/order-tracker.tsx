...
export function OrderTracker({ id }: { id: string }) {
  ...
  if (!mounted) {
    return <p className="py-20 text-center text-muted">Loading your order…</p>;
  }
  ...
  return (
    <div data-brand={order.brand} className="mx-auto max-w-2xl">
      <div className="text-center">
        ...
        <p className="label mt-3 text-brand">#{order.number}</p>
      </div>
                ...
                <div className="flex flex-col items-center">
                  <span
                    className={cn(
                      ...
                      done || active ? "border-brand bg-brand text-on-brand" : "border-line text-muted",
                    )}
                  >
                    {done ? <CheckCircle2 className="size-4" /> : <span className="size-2 rounded-full bg-current" />}
                  </span>
                  {idx < ORDER_STEPS.length - 1 && (
                    <span className={cn("min-h-6 w-px flex-1", idx < stepIndex ? "bg-brand" : "bg-line")} />
                  )}
                </div>
                <div className="pb-6">
                  <p className={cn(active || done ? "text-ink" : "text-muted", active && "font-medium")}>
                    {label}
                  </p>
                  {active && <p className="text-xs text-brand">In progress…</p>}
                </div>
              </li>
            );
          })}
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2">
        <div className="rounded-2xl border border-line bg-surface p-5 text-sm">
          <h3 className="label text-muted">{order.fulfilment === "delivery" ? "Delivery" : "Pickup"}</h3>
          <div className="mt-3 flex items-start gap-2">
            {order.fulfilment === "delivery" ? (
              <Bike className="mt-0.5 size-4 shrink-0 text-brand" />
            ) : (
            ...
            <span>
              {order.fulfilment === "delivery" ? (
                <>
                  {order.address}
                  <span className="block text-muted">{order.zoneName}</span>
                </>
              ) : (
